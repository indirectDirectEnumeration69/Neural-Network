...
public:
    ...
    class InputLayer : public RawInputX { //sort out T,T pairs into x1-x3 
    public:
        InputLayer() {
            auto pairs = RawInputX::RawInput();
            for (auto& relation : pairs) {
                x1.push_back(relation[0]);
                x2.push_back(relation[1]);
                if (relation != relation[0] && relation[1]) {
                    auto& Nextrelation = *(++ &relation);
                    x3.push_back(Nextrelation);
                }
                else {
                    x3.push_back(0);
                }
